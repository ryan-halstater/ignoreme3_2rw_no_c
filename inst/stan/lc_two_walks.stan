...
model {
  ...
  target += normal_lpdf(ks1[1]|0,sigma1);
  target += normal_lpdf(ks1[2:(T-1)]|ks1[1:(T- 2)],sigma1);    // Random walk with drift prior
  
  target += normal_lpdf(ks2[1]|0,sigma2);
  target += normal_lpdf(ks2[2:(T-1)]|ks2[1:(T- 2)],sigma2);
...
generated quantities {
  ...
  int pos2= 1;
  int pos3= 1;
  k_p1[1] = k1[T]+sigma1 * normal_rng(0,1);
  k_p2[1] = k2[T]+sigma2 * normal_rng(0,1);
  for (t in 2:Tfor) k_p1[t] = k_p1[t - 1] + sigma1 * normal_rng(0,1);
  for (t in 2:Tfor) k_p2[t] = k_p2[t - 1] + sigma2 * normal_rng(0,1);
  if (family==0){
